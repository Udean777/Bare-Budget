        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    if (entities.isNotEmpty()) Result.success(entities.map { it.toTransaction() }) else Result.failure(ApiErrorParser.parse(response))
                }
            } catch (e: Exception) {
                val entities = db.transactionDao().getAllTransactions()
                if (entities.isNotEmpty()) Result.success(entities.map { it.toTransaction() }) else Result.failure(ApiErrorParser.fromThrowable(e))
            }
        }
    ...
    suspend fun createTransaction(request: CreateTransactionRequest): Result<Transaction> =
        withContext(Dispatchers.IO) {
            try {
                ...
                val dtoReq = com.ssajudn.barebudget.data.network.dto.CreateTransactionRequestDto(request.amount, request.type.name, request.category.name, request.merchant, dateStr, request.notes ?: "", request.receiptUrl, request.walletId, request.toWalletId)
                val response = api.createTransaction(dtoReq)
                if (response.isSuccessful && response.body() != null) {

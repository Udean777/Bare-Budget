...
class WalletRemoteDataSource @Inject constructor(
    ...
    suspend fun getWallets(): Result<List<Wallet>> = withContext(Dispatchers.IO) {
        try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                val wallets = dtos.map { it.toDomain() }
    ...
    suspend fun createWallet(request: CreateWalletRequest): Result<Wallet> = withContext(Dispatchers.IO) {
        try {
            val dtoReq = com.ssajudn.barebudget.data.network.dto.CreateWalletRequestDto(request.name, request.balance, request.colorHex, request.iconName)
            val response = api.createWallet(dtoReq)
            if (response.isSuccessful && response.body() != null) {
                val wallet = response.body()!!.toDomain()
